...
struct UAV
{
	...
	void use_radar(const Target& Obj, UAV& D, UAV& F) // обследование пространства
	{
		...
		if (abs(x - Obj.x) <= capture_range && abs(y - Obj.y) <= capture_range && abs(z- Obj.z) <= capture_range)
		{
			target.distance = calculate_distance(Obj.x, Obj.y, Obj.z);
			target.heading = calculate_heading(Obj.x, Obj.y);
			double l = sqrt( pow( (x-Obj.x),2) + pow((y-Obj.y),2)     );
			target.angle_of_attack = calculate_angle_of_attack(l,Obj.z);
...
int main()
{
	...
	double t = 0;
	srand(time(0));
	double pursuiting_time = 10;
	double time_of_pursuit = 0;
	...
	{	
		...
		cout << "Time: " << t << endl << time_of_pursuit << endl;
		...
		{
			if (time_of_pursuit == 0)
			{
				cout << "target is captured at " << t << endl;
				//system("pause");
			}
			time_of_pursuit += 0.1;
		}
		if ((int)time_of_pursuit == (int)pursuiting_time)
		{
